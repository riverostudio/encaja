...
export async function obtenerResumenAdmin(periodoDias: number): Promise<ResumenAdmin> {
  ...
  if (!sql) {
    ...
    return {
      ...
      periodoDias: dias,
      resumen: {
        activosAhora: sesiones.filter(([, s]) => ahora - s.ultima.getTime() < 120_000).length,
      ...
      activos: sesiones
        .filter(([, s]) => ahora - s.ultima.getTime() < 120_000)
        .map(([id, s]) => ({
          sesion: id.slice(0, 8),
          pagina: s.pagina,
          segundos: s.duracion,
  ...
  const [totales, tipos, paginas, categorias, ayudas, serie, activos, recientes] = await sql.transaction([
    sql`
      SELECT
        ...
        (SELECT COUNT(*) FROM sesiones_analitica WHERE ultima_actividad >= NOW() - INTERVAL '2 minutes') AS activos,
    ...
    `,
    sql`SELECT LEFT(sesion_hash, 8) AS sesion, pagina, duracion_segundos AS segundos, ultima_actividad AS ultima FROM sesiones_analitica WHERE ultima_actividad >= NOW() - INTERVAL '2 minutes' ORDER BY ultima_actividad DESC LIMIT 100`,
